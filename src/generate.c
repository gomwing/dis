...
USEOPTION option_q, option_r, option_x, option_B, option_M, option_N,   /* option_Q, */
  option_S, option_U;
...
static INLINE char *strcpy2(char *dst, char *src) {
	while((*dst++ = *src++) != 0);
	return --dst;
}
...
*/
extern void generate(char *xfilename, char *sfilename, time_t filedate, int argc, char *argv[]) {
...
static INLINE char *tab_out(int tab, char *buf) {
	int len;
	unsigned char c;
...
*/
private address textgen(address pc, address pcend) {
	...
	while(pc < pcend) {
		...
		if(code.flag == OTHER) {
			...
			if((Generate_SizeMode == SIZE_NORMAL && size != NOTHING && check_displacement(pc, &code))   /* -b0: 可能ならサイズ省略 */
...
#define OS9CALL(n, v, t)  (peekw (store0) == (n) && (v) < 0x100 && (l = (t)[(v)]))
		...
		if(*(UBYTE *) store0 == 0x70    /* moveq #imm,d0 + trap #15 なら */
		   ...
		   && IOCSlabel && (l = IOCSlabel[*(UBYTE *) (store0 + 1)]) != NULL && (pc < pcend)) {
...
extern void modify_operand(disasm * code) {
	...
	if(code->op1.operand[0]) {
		if(sp_a7_flag)
			a7toSP(&code->op1);
		if(upper_flag)
			strupr(code->op1.operand);
		labelchange(code, &code->op1);
		if(Old_syntax)
			syntax_new_to_old(&code->op1);

		if(code->op2.operand[0]) {
			if(sp_a7_flag)
				a7toSP(&code->op2);
			if(upper_flag)
				strupr(code->op2.operand);
			labelchange(code, &code->op2);
			if(Old_syntax)
				syntax_new_to_old(&code->op2);

			if(code->op3.operand[0]) {
				if(sp_a7_flag)
					a7toSP(&code->op3);
				if(upper_flag)
					strupr(code->op3.operand);
				labelchange(code, &code->op3);
				if(Old_syntax)
					syntax_new_to_old(&code->op3);

				if(code->op4.operand[0]) {
					if(sp_a7_flag)
						a7toSP(&code->op4);
					if(upper_flag)
						strupr(code->op4.operand);
					labelchange(code, &code->op4);
					if(Old_syntax)
						syntax_new_to_old(&code->op4);
				}
			}
		}
	}
...
*/
private void a7toSP(operand * op) {
	char *ptr = op->operand;
...
*/
private address datagen(address pc, address pcend, opesize size) {
	address next_adrs;
...
*/
private void datagen_sub(address pc, address pcend, opesize size) {
	switch (size) {
		case STRING:
			strgen(pc, pcend);
			break;
...
*/
private void dataout(address pc, ULONG byte, opesize size) {
	address store = pc + Ofst;
...
*/
private void byteout(address pc, ULONG byte, boolean roption_flag) {
	address store = pc + Ofst;

#ifndef OSKDIS                  /* OS-9/68K の r68 には dcb に相当する疑似命令が無い */
	if(!roption_flag && (byte >= 1 * 2) && Compress_len && (byte >= Compress_len)) {
...
private void wordout(address pc, ULONG byte) {
	address store = pc + Ofst;
...
private void longout(address pc, ULONG byte) {
	address store = pc + Ofst;
...
static INLINE boolean is_sb(unsigned char *ptr) {
	return (isprkana(ptr[0]) && ptr[0] != QUOTE_CHAR);
}

static INLINE boolean is_mb_zen(unsigned char *ptr) {
	return (iskanji(ptr[0]) && iskanji2(ptr[1]));
}

static INLINE boolean is_mb_han(unsigned char *ptr) {
...
private void strgen(address pc, address pcend) {
...
*/
private void relgen(address pc, address pcend) {
...
*/
private void rellonggen(address pc, address pcend) {
	char buf[256], tabletop[128];
	char *bufp;
	const address pc0 = pc;
...
private address tablegen(address pc, address pcend, lblmode mode) {
	int loop, i;
	table *tableptr;
	...
	for(loop = 0; loop < tableptr->loop; loop++) {  /* implicit loop */
		for(i = 0; i < tableptr->lines; i++) {  /* line# loop */
			...
			pc = tablegen_sub(&tableptr->formulaptr[i], tableptr->tabletop, pc, i + 1 < tableptr->lines);
#if 0
...
private address tablegen_sub(formula * exprptr, address tabletop, address pc, int notlast) {
...
static void tablegen_dc(address nextlabel, lblbuf * lptr, char *op, int size) {
...
*/
private void tablegen_label(address pc, address pcend, lblbuf * lptr) {
	address nlabel = lptr->label;
	lblmode nmode = lptr->mode;

	DEBUG_PRINT(("tablegen_label(%x,%x,%x)\n", (unsigned int)pc, (unsigned int)pcend, (unsigned int)nlabel));
...
private void byteout_for_moption(address pc, char *buffer, int size) {
	char *p;
	address store = pc + Ofst;
...
*/
extern void byteout_for_xoption(address pc, ULONG byte, char *buffer) {
	int i;
	char c;
	address store = pc + Ofst;
...
*/
private void bssgen(address pc, address nlabel, opesize size) {
	ULONG byte;
...
private void labelchange(disasm * code, operand * op) {
	...
	if(op->opval != (address) - 1 &&
	   (op->labelchange1 || ((op->ea == AbLong || ((op->ea == IMMED) && (code->size2 == LONGSIZE)))
...
/* ラベル定義行のコロン出力 */
static INLINE void add_colon(char *ptr, int xdef) {
	if(SymbolColonNum) {
		*ptr++ = ':';
		/* -C3 又は、-C2 かつ外部定義なら :: */
...
*/
private void label_op_out(address adrs) {
	char buf[128];
...
*/
extern void make_proper_symbol(char *buf, address adrs) {
	address arg1;
	LONG shift;
...
*/
extern char *make_symbol(char *ptr, address adrs, LONG sft) {
	symbol *symbol_ptr;
...
private const char *ctimez(time_t * clock) {
	char *p = ctime(clock);
	char *n;
...
static INLINE FILE *open_header_file(void) {
	FILE *fp = NULL;
	const char *fname = Header_filename ? Header_filename : getenv("dis_header");

	if(fname && *fname && (fp = fopen(fname, "rt")) == NULL) {
#if 0
		eprintf("\n" "ヘッダ記述ファイル \"%s\" がオープン出来ません.\n" "標準の内容を出力します.\n", file);
#else
		eprintf("\nheader description file cannot be opened.\n");
#endif
	}
...
*/
private void makeheader(char *filename, time_t filedate, char *argv0, char *comline) {
	char buffer[256];
	char *envptr;
	char cc = CommentChar;
	...
	outputf("%c  Owner       %d.%d" CR, cc, HeadOSK.owner >> 16, HeadOSK.owner & 0xffff);
	...
	switch (HeadOSK.type & 0x0f00) {
		...
		case 0x0e00:            /* Driver     */
			outputf("\tpsect\t%s,$%04x,$%04x,%d,%d,L%06x",
					buffer, HeadOSK.type, HeadOSK.attr, HeadOSK.edition, 0, HeadOSK.exec);
			if(HeadOSK.excpt)
				outputf(",L%06x", HeadOSK.excpt);
			break;
	}
...
#ifndef OSKDIS
	{                           /* 外部定義シンボルの出力 */
		char *colon = (SymbolColonNum == 0) ? "" : (SymbolColonNum == 1) ? ":" : "::";
...
*/
private void output_opecode(char *ptr) {
	char buf[32];
...
private void oskdis_gen(lblbuf * lptr) {
...
*/
private address idatagen_sub(address pc, address wkpc, address pcend, lblmode mode) {
			...
			case LONGSIZE:
				...
				if(mode & CODEPTR) {
					...
					char *s = (x == 0) ? PSEUDO DC_L "_btext\t* $%08x" CR
						: (x == HeadOSK.name) ? PSEUDO DC_L "_bname\t* $%08x" CR : PSEUDO DC_L "L%06x" CR;
					outputf(s, x);
...
*/
private void vlabelout(address adrs, lblmode mode) {
	char buff[128];
